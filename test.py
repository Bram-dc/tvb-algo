...
from parallel_jit import simulation as parallel_jit_simulation
...
for i, speed in tqdm(enumerate([1.0, 2.0, 10.0])):
    ...
    T_parallel_jit, Xs_parallel_jit, _ = parallel_jit_simulation.simulate(
        W_list, D_list, dt, tf, k, speed, freq
    )
    ...
    for x_base, x_original, x_base_single_ncv, x_parallel, x_parallel_jit in zip(
        Xs_base, Xs_original, Xs_base_single_ncv, Xs_parallel, Xs_parallel_jit
    ):
        assert (
            len(x_base)
            == len(x_original)
        ...
        for (
            ...
            xi_parallel,
            xi_parallel_jit,
        ) in zip(x_base, x_original, x_base_single_ncv, x_parallel, x_parallel_jit):
            assert np.allclose(
            ...
            assert np.allclose(
                xi_original, xi_parallel_jit, atol=tolerance
            ), f"Mismatch in Xs for speed {speed} at {xi_original} vs {xi_parallel_jit} (original vs parallel_jit)"
